...
class AudioService {
  ...
  Future<String> generateSpeech({
    ...
    try {
      final response = await _replicateService.createPrediction(
        model: _barkModelId,
        input: {
          'text': text,
      ...
      while (audioUrl == null) {
        ...
        final status = await _replicateService.getPrediction(predictionId);
        ...
        if (status['status'] == 'succeeded') {
          ...
          // The first output is the audio file URL
          audioUrl = outputs.first as String;
          break;
      ...
      final savedPath = await _downloadAndSaveAudio(audioUrl);
      return savedPath;
    } catch (e) {
